...
export default function OutfitsScreen() {
  ...
  const generate = async () => {
    ...
    try {
      ...
      // Warn if no items match the occasion, but still try — LLM will do its best
      const matchingItems = items.filter((i: any) => i.occasions?.includes(occasion));
      if (matchingItems.length === 0) {
        Alert.alert(
          `No ${occasion} items`,
          `You don't have any items tagged for ${occasion}. Try adding some, or we'll suggest the closest alternatives from your closet.`,
          [
  ...
  const doGenerate = async (items: any[], city: string) => {
    try {
      // Get real city from device location
      ...
      if (!result.outfits || result.outfits.length === 0) {
        Alert.alert(
          'No outfits found',
          `Couldn't build a ${occasion} outfit from your current closet. Try adding more items or a different occasion.`
        );
        setPhase('pick');
        return;
      }
